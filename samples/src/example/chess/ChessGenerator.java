...
// remove reference to Board (space,mflag)
// replaced with Board (Space, SetSpace, Flag, ang SetFlag)

// changed June 16, 1999 by RL
// change reference to from_row, from_col, to_row, to_col as methods
// change constructors for Move definitions

/**
  A simple move that just moves a piece from a source location
  to a destination location, possibly with a capture.
 */
class SimpleMove extends Move {
  ...
  /**
    Construct a move that moves the piece from f_row, f_col to
    t_row, t_col
   */
  ...
  public void apply( Board b )
  {
    ...
    saved_piece = b.Space(to_row(),to_col());
    epsq = b.getEPsq();
    b.setEPsq(0);
    if (piece() == Board.B_PAWN || piece() == Board.R_PAWN) {
       if (from_row()+2 == to_row()) b.setEPsq(((from_row()+1) << 3) | from_col());
       else if (from_row()-2 == to_row()) b.setEPsq(((from_row()-1) << 3) | from_col());
    }

    // Make copies of the source and destination move flags.
    to_flag = b.Flag(to_row(), to_col());
    from_flag = b.Flag(from_row(), from_col());
    ...
    b.SetSpace(to_row(), to_col(), b.Space(from_row(), from_col()));

    // Empty the from position
    b.SetSpace(from_row(), from_col(), Board.EMPTY);

    // Added June 17, 1999 by RL
    if (!b.thinking()) display();
  ...
  public void undo( Board b )
  {
    ...
    b.SetSpace(from_row(), from_col(), b.Space(to_row(), to_col()));

    // Empty the to position
    b.SetSpace(to_row(), to_col(), saved_piece);
...
/**
  A special move that moves a pawn and promotes it to a queen at the
  same time.  This code assumes that the pawn is stored as a value
  of 1.
 */
class PromoteMove extends Move {
  ...
  {
    super(b, f_row, f_col, t_row, t_col, b.Space(f_row, f_col) *
      Board.B_QUEEN);
  }
  ...
  /**
    Construct a move that moves the piece from f_row, f_col to
    t_row, t_col and promoted to a p_promote
   */
  PromoteMove(Board b, int f_row, int f_col, int t_row, int t_col, int p_promote )
  {
  ...
  public void undo( Board b )
  {
    ...
    // Put a copy of the piece in the back in the from position
    b.SetSpace(from_row(), from_col(), piece());
...
/**
  A CastleMove performs the castle operation between a king and the
  rook at the other end of the move.
 */
class CastleMove extends Move {
  ...
  public void apply( Board b )
  {
    ...
    if( to_col() < from_col() ){
      b.SetSpace(to_row(), from_col() - 2, b.Space(from_row(), from_col()));
      b.SetSpace(to_row(), from_col() - 1, b.Space(to_row(), 0));

      // Original position of rook made empty.
      b.SetSpace(to_row(), 0, Board.EMPTY);

      // Flag rook space as having been moved.
      b.SetFlag(to_row(), 0, true);
    } else {
      b.SetSpace(to_row(), from_col() + 2, b.Space(from_row(), from_col()));
      b.SetSpace(to_row(), from_col() + 1, b.Space(to_row(), Board.WIDTH-1));

      // Original position of rook made empty.
      b.SetSpace(to_row(), Board.WIDTH-1, Board.EMPTY);
  ...
  public void undo( Board b )
  {
    ...
    if( to_col() < from_col() ){
      b.SetSpace(to_row(), 0, b.Space(to_row(), from_col() - 1));
      b.SetSpace(from_row(), from_col(), b.Space(to_row(), from_col() - 2));

      b.SetSpace(to_row(), from_col() - 2, Board.EMPTY);
      b.SetSpace(to_row(), from_col() - 1, Board.EMPTY);

      // Restore the move flags for rook space
      b.SetFlag(to_row(), 0, false);
    } else {
      b.SetSpace(to_row(), Board.WIDTH-1, b.Space(to_row(), from_col() + 1));
      b.SetSpace(from_row(), from_col(), b.Space(to_row() ,from_col() + 2));
...
class EPMove extends Move {
  ...
  public void apply( Board b )
  {
    ...
    saved_piece = b.Space(from_row(), epsq & 07);
    b.SetSpace(from_row(), epsq & 07 , Board.EMPTY);
    b.SetSpace(to_row(), to_col() , piece());
    b.SetSpace(from_row(), from_col() , Board.EMPTY);
  ...
  public void undo( Board b )
  {
    ...
    b.SetSpace(from_row(), from_col() , piece());
    b.SetSpace(to_row(), to_col() , Board.EMPTY);
    b.SetSpace(from_row(), epsq & 07 , saved_piece);
...
public class ChessGenerator extends Generator {
  ...
  // the result.
  private Move movePawn( Move move_list, Board b, int side,
                            int row, int col )
  {
    ...
    if( row + side >= 0 && row + side < Board.HEIGHT ){
      if( b.Space(row + side, col) == Board.EMPTY ){
        Move m;
        ...
        if( ( side == Board.B_SIDE && row == 1 ) ||
          ...
          if( b.Space(row + side * 2, col) == Board.EMPTY ){
      ...
      // First, to the left.
      if( col > 0 && b.Space(row + side, col - 1) * side < 0 ){
        Move m;
        // See if we get to promote the pawn
        if( row + side == 0 || row + side == Board.HEIGHT - 1 )
  ...
  private Move moveKnight( Move move_list, Board b, int side,
                           int row, int col )
  {
    if( row > 0 ){
      // See if the destination space is empty or has an enemy piece.
      if( col - 2 >= 0 && b.Space( row - 1, col - 2) * side <= 0 ){
        Move m = new SimpleMove( b, row, col, row - 1, col - 2 );
        m.next = move_list;
        move_list = m;
      }
      // See if the destination space is empty or has an enemy piece.
      if( col + 2 < Board.WIDTH && b.Space( row - 1, col + 2) * side <= 0 ){
        Move m = new SimpleMove( b, row, col, row - 1, col + 2 );
        m.next = move_list;
        move_list = m;
      }
      ...
      if( row - 1 > 0 ){
        // See if the destination space is empty or has an enemy piece.
        if( col > 0 && b.Space(row - 2, col - 1) * side <= 0 ){
          Move m = new SimpleMove( b, row, col, row - 2, col - 1 );
          m.next = move_list;
          move_list = m;
        }
        // See if the destination space is empty or has an enemy piece.
        if( col + 1 < Board.WIDTH && b.Space(row - 2, col + 1) * side <= 0 ){
          Move m = new SimpleMove( b, row, col, row - 2, col + 1 );
          m.next = move_list;
          move_list = m;
        }
  ...
  private Move moveKing( Move move_list, Board b, int side,
                           int row, int col )
  {
    // Move straight up or diagonal
    if( row > 0 ){
      if( b.Space( row - 1, col) * side <= 0 ){
        Move m = new SimpleMove( b, row, col, row - 1, col );
        m.next = move_list;
        move_list = m;
      }

      if( col > 0 && b.Space(row - 1 ,col - 1) * side <= 0 ){
        Move m = new SimpleMove( b, row, col, row - 1, col - 1 );
        m.next = move_list;
        move_list = m;
      }

      if( col + 1 < Board.WIDTH && b.Space(row - 1, col + 1) * side <= 0 ){
        Move m = new SimpleMove( b, row, col, row - 1, col + 1 );
        m.next = move_list;
        move_list = m;
      }
    ...
    // Move left or right
    if( col > 0 && b.Space( row , col - 1 ) * side <= 0 ){
      Move m = new SimpleMove( b, row, col, row, col - 1 );
      m.next = move_list;
      move_list = m;
    }

    if( col + 1 < Board.WIDTH && b.Space( row , col + 1 ) * side <= 0 ){
      Move m = new SimpleMove( b, row, col, row, col + 1 );
      m.next = move_list;
      move_list = m;
    }
    ...
    if( b.Flag( row , col ) == false ){
      ...
      if( b.Flag( row , Board.WIDTH - 1 ) == false ){
        // Make sure the path between the king and the rook is vacant.
        boolean vacant = true;
        ...
        if( vacant ){
          ...
          if( vacant ){
            // Move m = new CastleMove( b, row, col, row, Board.WIDTH - 1 );
            Move m = new CastleMove( b, row, col, row, col+2 );
            m.next = move_list;
            move_list = m;
          }
  ...
  protected Move moveDiagonal( Move move_list, Board b, int side,
                               int row, int col )
  {
    //left-up
    ...
    {   if (b.Space(row+tmprow,col+tmpcol) == 0)
        ...
        {
            if (b.Space(row+tmprow,col+tmpcol) * side < 0)
            {   Move m = new SimpleMove( b, row, col, row+tmprow, col+tmpcol);
                m.next = move_list;
                move_list = m;
            }
  ...
  protected Move moveRectangular( Move move_list, Board b, int side,
                                  int row, int col )
  {
    boolean NoPiece = true;
    //left
    ...
    {   if(b.Space(row,col+tmpcol) == 0)
        {   Move m = new SimpleMove( b, row, col, row, col+tmpcol);
         ...
         {
            if(b.Space(row,col+tmpcol) * side < 0)
            {   Move m = new SimpleMove( b, row, col, row, col+tmpcol);
                m.next = move_list;
                move_list = m;
            }
    ...
    {   if(b.Space(row+tmprow,col) == 0)
        {   Move m = new SimpleMove( b, row, col, row+tmprow, col);
         ...
         {
            if(b.Space(row+tmprow,col) * side < 0)
            {   Move m = new SimpleMove( b, row, col, row+tmprow, col);
                m.next = move_list;
                move_list = m;
            }
  ...
  private boolean threatened( Board b, int side, int row, int col )
  {
    ...
    for( m = move_list; m != null; m = m.next )
      if( b.Space( m.to_row() , m.to_col() ) * side == Board.R_PAWN )
        return true;

    move_list = moveRectangular( null, b, side, row, col );
    for( m = move_list; m != null; m = m.next )
      if( b.Space( m.to_row() , m.to_col() ) * side == Board.R_ROOK ||
          b.Space( m.to_row() , m.to_col() ) * side == Board.R_QUEEN )
        return true;

    move_list = moveDiagonal( null, b, side, row, col );
    for( m = move_list; m != null; m = m.next )
      if( b.Space( m.to_row() , m.to_col() ) * side == Board.R_BISHOP ||
          b.Space( m.to_row() , m.to_col() ) * side == Board.R_QUEEN )
        return true;

    move_list = moveKnight( null, b, side, row, col );
    for( m = move_list; m != null; m = m.next )
      if( b.Space( m.to_row() , m.to_col() ) * side == Board.R_KNIGHT )
        return true;

    // Temporarily set the moved flag so we don't try to castle from
    // inside this function
    boolean saved_flag = b.Flag( row , col );
    b.SetFlag( row , col , true);

    move_list = moveKing( null, b, side, row, col );
    for( m = move_list; m != null; m = m.next )
      if( b.Space( m.to_row() , m.to_col() ) * side == Board.R_KING ){
