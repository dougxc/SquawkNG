...
public class Move {
  ...
  public Move(Board b, int from_r, int from_c, int to_r, int to_c)
  {
    ...
    t_row = to_r;
    t_col = to_c;
      p_piece = b.Space(f_row, f_col);
      if (castle())
        p_captured = Board.EMPTY;
      else
        p_captured = b.Space(t_row, t_col);
      p_promote = Board.EMPTY;
      if ((p_piece == Board.R_PAWN || p_piece == Board.B_PAWN) &&
          p_captured == Board.EMPTY && b.getEPsq() == ((t_row<<3) | t_col)) {
         p_captured = -p_piece;
         ep_flag = true;
      }
      m_pvc = 0;
  ...
  /**
     Return true if this move is defined (i.e. if it is a class
     derived from Move rather than an instance of Move itself.
   */
  ...
  /**
    ...
    Classes derived from Move can override this function
    to make more interesting moves.
   */
  ...
  /**
    Modify the given board to return it to the state before
    this move was made.
  ...
  // Added June 16, 1999 by RL
  int from_row(){ return f_row; }
  int from_col(){ return f_col; }
  int to_row()  { return t_row; }
  int to_col()  { return t_col; }
  ...
  {
     char[] p = {'K','Q','B','N','R','P',' ','P','R','N','B','Q','K' };
  ...
  {
    return ((p_piece == Board.B_KING || p_piece == Board.R_KING) &&
         (t_col > f_col+1 || t_col < f_col-1 ));
  }
