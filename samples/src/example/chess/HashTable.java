...
public class HashTable {
  ...
  */
  ...
  public int[] probe(long hkey, int ply, int depth, int side,
                     int alpha, int beta, int random)
  {
      long w1;
      long w2;
      int key;
      ...
      if (found) {
        ...
        switch (type) {
          case EXACT:
            if (val > Board.WIN_SCORE-100) val-=(ply-1);
            else if (val < -Board.WIN_SCORE+100) val+=(ply-1);
            else val += random*side;
            ret[0]=val;
            ret[2]=EXACT;
            break;
          case UPPER:
  ...
  public void store(long hkey, int pvc, int ply, int depth,
                    int side, int type, int val, int random)
  {
      ...
      if (type == EXACT) {
        if (val > Board.WIN_SCORE-100) val=val+ply-1;
        else if (val < -Board.WIN_SCORE+100) val=val-ply+1;
        else val -= random*side;
      } else val -= random*side;
      w1 = pvc&037777777;
      w1 = (w1<<21) | ((long) (val+Board.WIN_SCORE)& 07777777);
      w1 = (w1<<2) | ((long) (type&03));
      w1 = (w1<<7) | ((long) (depth&0177));
      w1 = (w1<<3) | ((long) (age&07));
      stores++;
      ...
      if (expired(a) || depth >= d) {
        ...
        key = (int) (w2 & (hash_size*2-1));
        if (worda2[key] != 0 && worda2[key] != w2 && age == (int)(worda1[key] & 07))
            collisions++;
         worda1[key] = w1;
         worda2[key] = w2;
      }
  ...
  public long hashVal(Board b)
  {
    ...
    for (i=0; i < 8; ++i)
      for (j=0; j < 8; ++j)
         hkey = hashVal(hkey, b.Space(i,j), i, j);
