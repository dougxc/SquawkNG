    ...
    public void dispatch(int event, DataInput in) throws IOException {
        switch(event) {
            case penDownKVMEvent:
                penDown(in.readInt(), in.readInt());
                break;

            case penUpKVMEvent:
                penUp(in.readInt(), in.readInt());
                break;

            case penMoveKVMEvent:
                penMove(in.readInt(), in.readInt());
                break;
            ...
            case beamReceiveKVMEvent: {
                com.sun.squawk.io.j2me.events.Protocol e =
                    (com.sun.squawk.io.j2me.events.Protocol)con;
                beamReceive(e.readByteArray(in));
                break;
            }

            default:
                unknownEvent(event, in);
                break;
        }
    }
