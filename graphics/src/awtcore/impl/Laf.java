...
public class Laf {
    ...
    public void drawWindow (Graphics g, int w, int h,
        ...
        if (type == WINDOW_FRAME) {
            ...
            if (title != null) {
                //drawFocusString (g, Color.blue, 0, 0, title, focus);
                //g.drawLine (0, 8, 1000, 8);
                FontMetrics fm = g.getFontMetrics ();
                Color fg = g.getColor ();
                g.setColor (SystemColor.activeCaption);
                g.fillRect (0, 0, w, fm.getHeight());
                g.setColor (Color.white);
                g.drawString (title, 0, 0+fm.getAscent ());
                g.setColor (fg);
            }
        }
    }
