...
public class IntHashtable {
     ...
     * value of this field is (int)(capacity * loadFactor)/100.)
     */
    protected int threshold;
    /**
     * The load factor for the hashtable.
     */
    protected int loadFactorPercent;
     ...
     *
     * @param  initialCapacity  the initial capacity of the hashtable
     * @param  loadFactorPercent       a number between 0 and 99
     * @exception  IllegalArgumentException  if the initial capacity is
     *             less than zero, if the load factor is less than or
     *             equal to zero, or if the load factor is greater than
     *             one.
     */
    public IntHashtable(int initialCapacity, int loadFactorPercent) {
    if (initialCapacity < 0) {
        throw new IllegalArgumentException("Illegal Capacity: " + initialCapacity);
        }
        if ((loadFactorPercent > 99) || (loadFactorPercent <= 0)) {
            throw new IllegalArgumentException("Illegal Load: " + loadFactorPercent);
        }
        if (initialCapacity == 0) {
            initialCapacity = 1;
        }
...
//**FLOAT**      double p = k * 0.6180339887;
//**FLOAT**      return (int)(m * (p - Math.floor(p)));
        if (k < 0) {
           k = -k;
        }
        return k % m;  //**FLOAT**
