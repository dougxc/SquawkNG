/*
 * @(#)GenerateOopMap.java	1.4 02/01/09 14:44:00
 *
...
public class GenerateOopMap {
  ...
  static class RetTable {
    ...
    void computeRetTable(Method method) {
      BytecodeStream i = new BytecodeStream(method.getCode());
      int bytecode;

      while(i.next()) {
        bytecode = i.getBytecode();
        switch (bytecode) {
        case Bytecodes._jsr:
          addJsr(i.getNextBci(), i.getDestination(1, false));
          break;
        case Bytecodes._jsr_w:
          addJsr(i.getNextBci(), i.getDestination(1, true));
          break;	    
        } 
      }  
    }
  ...
  void            push                       (CellTypeState cts) {
    if ( _stack_top >= _max_stack) {
      if (DEBUG) {
        System.err.println("Method: " + method().getName().toString() + method().getSignature().toString() +
  ...
  void            printCurrentState          (PrintStream tty,
                                              ...
                                              boolean        detailed) {
    if (detailed) {
      tty.print("     " + currentBC.getBci() + " vars     = ");
      printStates(tty, vars(), _max_locals);
      tty.print("    " + Bytecodes.getName(currentBC.getBytecode()));
      switch(currentBC.getBytecode()) {
      ...
      case Bytecodes._invokeinterface:   
        ...
        int idx = currentBC.getUnsigned(1, 2);
        tty.print(" idx " + idx);
        /*
          int idx = currentBC.getUnsigned(1, 2);
      ...
      if (_monitor_top != bad_monitors) {
        tty.print("  monitors = '" + stateVecToString(monitors(), _monitor_top) + "'  \t" +
                  Bytecodes.getName(currentBC.getBytecode()));
      } else {
        tty.print("  [bad monitor stack]");
      }
      switch(currentBC.getBytecode()) {
  ...
  void            reportMonitorMismatch      (String msg) {
    if (Assert.ASSERTS_ENABLED) {
      ...
      System.err.print(method().getName().toString());
  ...
  void          markBBHeadersAndCountGCPoints() {
    ...
    ListIterator excps = method().getExceptionHandlers().listIterator();
    while (excps.hasNext()) {
      ExceptionHandler handler = (ExceptionHandler) excps.next();
      markBB(handler.getTarget(), null);
    }

    // Then iterate through the code
    BytecodeStream bcs = new BytecodeStream(_method.getCode());
    int bytecode;

    while(bcs.next()) {
      bytecode = bcs.getBytecode();
      int bci = bcs.getBci();
      ...
      case Bytecodes._jsr:
        ...
        markBB(bci + Bytecodes.getLength(bytecode), null); 
        break;
      case Bytecodes._jsr_w:
        if (Assert.ASSERTS_ENABLED) {
          Assert.that(!fellThrough, "should not happen");
  ...
  void          interpBB                    (BasicBlock bb) {
    ...
    BytecodeStream itr = new BytecodeStream(_method.getCode());  
    ...
    while(itr.getNextBci() < lim_bci && !_got_error) {      
    ...
    if (!_got_error) {
      if (Assert.ASSERTS_ENABLED) {
        Assert.that(itr.getNextBci() == lim_bci, "must point to end");
      }
      if (_has_exceptions || (_monitor_top != 0)) {
        doExceptionEdge(itr);
      }
      ...
      if (itr.getBytecode() == Bytecodes._ret) {
        if (Assert.ASSERTS_ENABLED) {
  ...
  void          markReachableCode() {
    ...
    while (excps.hasNext()) {
      ExceptionHandler handler = (ExceptionHandler) excps.next();
      BasicBlock bb = getBasicBlockAt(handler.getTarget());
    ...
    while (change[0] != 0) {
      ...
      for (int i = 0; i < _bb_count; i++) {      
        ...
        if (bb.isAlive()) {         
          ...
          bcs.reset(bb._end_bci); 
          bcs.next();
          int bytecode = bcs.getBytecode();
          int bci = bcs.getBci();
          if (Assert.ASSERTS_ENABLED) {
          ...
          case Bytecodes._jsr_w:
            ...
            reachableBasicblock(bci + Bytecodes.getLength(bytecode), change);
            break;          
          }
          if (fell_through) {
  ...
  void  initBasicBlocks                     () {
    ...
    BytecodeStream j = new BytecodeStream(_method.getCode());
    // int bytecode;
    ...
    int prev_bci = -1;
    while(j.next()) {
      if (j.getBytecode() == Bytecodes._monitorenter) {
        monitor_count++;
      }
  ...
  void  interp1                             (BytecodeStream itr) {
    if (DEBUG) {
      System.err.println(" - bci " + itr.getBci());
    }
    ...
    case Bytecodes._new:               ppush1(CellTypeState.makeLineRef(itr.getBci()));
      break; 
    ...
    case Bytecodes._ldc:               doLdc(itr.getUnsigned(1, 1), itr.getBci());    break; 
    case Bytecodes._ldc_w:             doLdc(itr.getUnsigned(1, 2), itr.getBci());break; 
    
    case Bytecodes._iload:             
    case Bytecodes._fload:             ppload(vCTS, itr.getUnsigned(1, 1)); break; 

    case Bytecodes._lload:             
    case Bytecodes._dload:             ppload(vvCTS,itr.getUnsigned(1, 1)); break; 

    case Bytecodes._aload:             ppload(rCTS, itr.getUnsigned(1, 1)); break;
    ...
    case Bytecodes._aaload:            ppNewRef(vrCTS, itr.getBci()); break;
    
    case Bytecodes._istore:            
    case Bytecodes._fstore:            ppstore(vCTS, itr.getUnsigned(1, 1)); break;

    case Bytecodes._lstore:            
    case Bytecodes._dstore:            ppstore(vvCTS, itr.getUnsigned(1, 1)); break;

    case Bytecodes._astore:            doAstore(itr.getUnsigned(1, 1));     break;
    ...
    case Bytecodes._jsr:               doJsr(itr.getDestination(1, false));          break;
    case Bytecodes._jsr_w:             doJsr(itr.getDestination(1, true));        break;
    ...
    case Bytecodes._fast_putstatic:
    case Bytecodes._putstatic:         doField(false, true,  itr.getUnsigned(1, 2), itr.getBci()); break; 
    case Bytecodes._getfield:          doField(true,  false, itr.getUnsigned(1, 2), itr.getBci()); break;
    case Bytecodes._putfield:          doField(false, false, itr.getUnsigned(1, 2), itr.getBci()); break;

    case Bytecodes._invokevirtual:
    case Bytecodes._invokespecial:     doMethod(false, false, itr.getUnsigned(1, 2), itr.getBci()); break;
    case Bytecodes._invokestatic:      doMethod(true,  false, itr.getUnsigned(1, 2), itr.getBci()); break;
    case Bytecodes._invokeinterface:   doMethod(false, true,  itr.getUnsigned(1, 2), itr.getBci()); break;
    case Bytecodes._newarray:
    case Bytecodes._anewarray:         ppNewRef(vCTS, itr.getBci()); break;
    case Bytecodes._checkcast:         doCheckcast(); break;
    case Bytecodes._arraylength:       
    case Bytecodes._instanceof:        pp(rCTS, vCTS); break;
    case Bytecodes._monitorenter:      doMonitorenter(itr.getBci()); break;  
    case Bytecodes._monitorexit:       doMonitorexit(itr.getBci()); break;
    ...
    case Bytecodes._multianewarray:    doMultianewarray(itr.getSigned(3, 1), itr.getBci()); break;
    ...
    case Bytecodes._fast_aaccess_0:     ppNewRef(rCTS, itr.getBci());            break; // Pair bytecode for (aload_0, _fast_agetfield)
    ...
    case Bytecodes._fast_fgetfield:     pp(rCTS, vCTS);            break;

    case Bytecodes._fast_lgetfield:     
    case Bytecodes._fast_dgetfield:     pp(rCTS, vvCTS);           break;

    case Bytecodes._fast_agetfield:     ppNewRef(rCTS, itr.getBci());           break;                                      
    
    case Bytecodes._fast_lputfield:     
    case Bytecodes._fast_dputfield:     ppop(vvrCTS);              break;
    case Bytecodes._fast_iputfield:     
    case Bytecodes._fast_fputfield:     ppop(vrCTS);               break;  
    
    case Bytecodes._fast_aputfield:     ppop(rrCTS);               break;

    case Bytecodes._fast_aload_0:       ppload(rCTS,  0);          break;    

    case Bytecodes._fast_getfield:      doField(true,  false, itr.getUnsigned(1, 2), itr.getBci()); break;
    case Bytecodes._fast_putfield:      doField(false, false, itr.getUnsigned(1, 2), itr.getBci()); break;
    ...
    case Bytecodes._fast_dldc_w:        ppush(vvCTS);              break;

    default: 
      throw new RuntimeException("unexpected opcode: " + itr.getBytecode());
    }
  }

  void  doExceptionEdge                     (BytecodeStream itr) {
    // Only check exception edge, if bytecode can trap
    if (!Bytecodes.canTrap(itr.getBytecode())) return;
    switch (itr.getBytecode()) {
    ...
    if (_has_exceptions) {
      ...
      while (excps.hasNext()) {
        ExceptionHandler handler = (ExceptionHandler) excps.next();
        int handler_pc = handler.getTarget();
        int catch_type = handler.getTypeIndex();
  ...
  void  doLdc                               (int idx, int bci) {
    ConstantPool  cp  = method().getConstants();
    int           tag = cp.getTagAt(idx);
    CellTypeState cts = ((tag == ConstantPool.JVM_CONSTANT_STRING) || (tag == ConstantPool.JVM_CONSTANT_UNRESOLVED_STRING))
  ...
  void  doField                             (boolean is_get, boolean is_static, int idx, int bci) {
    ...
    ConstantPool cp        = method().getConstants();
    Symbol signature       = cp.getSignatureAt(idx);
    ...
    char sigch = signature.toString().charAt(0);
  ...
  boolean jumpTargetsDo                     (BytecodeStream bcs, JumpClosure closure, int[] data) {
    ...
    case Bytecodes._ifnonnull:      
      closure.process(this, bcs.getDestination(1, false), data); 
    ...
    case Bytecodes._goto_w:         
      closure.process(this, bcs.getDestination(1, true), data);    
      break;
    case Bytecodes._tableswitch:
      {
        int offset = ((bci + 4) & ~0x03) - bci;
        int lo = bcs.getSigned(offset + 4, 4);
        int hi = bcs.getSigned(offset + 8, 4);
        closure.process(this, bcs.getDestination(offset, true), data);
        offset += 12;
    ...
    case Bytecodes._lookupswitch:
      {
        int offset = ((bci + 4) & ~0x03) - bci;
        int npairs = bcs.getSigned(offset + 4, 4);
        closure.process(this, bcs.getDestination(offset, true), data);
        offset += 12;
  ...
  public void computeMap() {
    if (DEBUG) {
      System.err.println("*** GenerateOopMap: computing for " +
                         method().getHolder().getName().toString() + "." +
    ...
    _has_exceptions = method().hasExceptionHandlers();
